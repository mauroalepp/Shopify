...
const Card = (data) =>{
    ...
    const renderIcon = (id) => {
        ...
        if (isInCart) {
            return (
            <div className="bg-green-500 m-2 p-1 absolute top-0 right-0 flex justify-center items-center w-6 h-6 rounded-full">
                <CheckIcon className="h-6 w-6 text-white "></CheckIcon>
            </div>
        )
        } else {
            return (
                <div className="m-2 p-1 absolute top-0 right-0 flex justify-center items-center bg-white w-6 h-6 rounded-full"
                    onClick={(event) => {
